...
Potrace = (function(){
	...
	var processPath = function(pList, param, pOutput){
		...
		if (pOutput == "stencil") {
			outscads.push(["paths_array = [[", src_width, ", ", src_height, ", 1]"].join(NULL_STR));
		} else if (pOutput == "stamp") {
			outscads.push(["paths_array = [[", src_width, ", ", src_height, ", 2]"].join(NULL_STR));
		} else if (pOutput == "cookie") {
			outscads.push(["paths_array = [[", src_width, ", ", src_height, ", 3], "].join(NULL_STR));
		}
		...
		for(var i=0, len=pList.length; i<len; i++){
			...
			/*debug += i + " : isEven = " + pList_i.isEven + ", area = " + pList[i].area + "\n";*/
			
			if (closedPath.isEven && pOutput == "cookie" ) {
				continue;
			}
			...
			if(closedPath.hasOwnProperty(CLASS_ID) && closedPath.class_id == CLOSED_PATH){
				...
				if (pList[i].area > area_max) {
					largest = closedPath;
					bigger = closedPath.outscad();
					area_max = pList[i].area;
				}
				if (pOutput == "cookie") {
					continue;
				} else {
					outscads.push([",[", closedPath.isEven ? '0' : '1' ,",", closedPath.outscad(),"]"].join(NULL_STR));
				}
		...
		if ( pOutput == "cookie") {
			closedPathList = new ClosedPathList();
			closedPathList.$a.push(largest);
			closedPathList._outpath = largest.outpath();
			outscads.push(bigger);
		} 
		
		outscads.push(["];"].join(NULL_STR));
		closedPathList._outscad = outscads.join(NULL_STR);
		outscad = closedPathList._outscad;
	...
	/*
		...
		for(var i=0, len=pList.length; i<len; i++){
			...
			if(closedPath.hasOwnProperty(CLASS_ID) && closedPath.class_id == CLOSED_PATH){	
				...
				if (closedPath.isEven) {
					neg_array.push(closedPath.outscad());
				} else {
					pos_array.push(closedPath.outscad());
	...
	ClosedPath.prototype = {
		...
		sqlen:function (pFrom, pTo) {
			return (pTo.x-pFrom.x)*(pTo.x-pFrom.x)+(pTo.y-pFrom.y)*(pTo.y-pFrom.y);
		},
		fm:function (pNum) {
			var n = Math.floor(pNum*1000);
			return n/1000;
		},
		outscad:function() {
		if(this._outscad != undefined){
				return this._outscad;
			}
			var pathtextbits = [];
			var $a = this.$a;
			var pt = $a[$a.length - 1].c[2];
			pathtextbits.push(["[[", this.fm(pt.x), ",", this.fm(pt.y), "]"].join(NULL_STR));
			var sqlength = 0;
			var lp = {x:pt.x, y:pt.y}; // last point
			
			for(var i = 0, len=$a.length; i < len; i++){
				var c = $a[i];
				var cc = c.c;
				if(c.tag == POTRACE_CORNER){
					pathtextbits.push([",[", this.fm(cc[1].x), ",", this.fm(cc[1].y), "],[", this.fm(cc[2].x), ",", this.fm(cc[2].y), "]"].join(NULL_STR));
					sqlength += this.sqlen(lp, cc[1]);
					sqlength += this.sqlen(cc[1], cc[2]);
					lp.x = cc[2].x;
					lp.y = cc[2].y;
				}else{
					var p0 = this.getBezierPoint(pt, cc[0], cc[1], cc[2], 0);
					var p1 = this.getBezierPoint(pt, cc[0], cc[1], cc[2], 0.25);
					var p2 = this.getBezierPoint(pt, cc[0], cc[1], cc[2], 0.5);
					var p3 = this.getBezierPoint(pt, cc[0], cc[1], cc[2], 1);
					pathtextbits.push([",[",this.fm(p0.x),",",this.fm(p0.y),"],[",this.fm(p1.x),",",this.fm(p1.y),"],[",this.fm(p2.x),",",this.fm(p2.y),"],[",this.fm(p3.x),",",this.fm(p3.y),"]"].join(NULL_STR));
					var cnt = 0;	
					sqlength += this.sqlen(lp, p0);
					sqlength += this.sqlen(p0, p1);
					sqlength += this.sqlen(p1, p2);
					sqlength += this.sqlen(p2, p3);
			...
			pathtextbits.push(["]"].join(NULL_STR));
			this._outscad = pathtextbits.join(NULL_STR);			
			return this._outscad;
		},
		_outscad: undefined,
		_outpath: undefined
	...
	//public
	//////////////////////////////////////////////////////////
	var trace = function(img, pOutput){
		...
		return traceCanvas(canvas, pOutput);
	};
	var traceCanvas = function(canvas, pOutput){
		//画像の二値化(binarization)
		//グレースケール→
		var bmd = new BitMap(canvas);
		...
		var curveList = processPath(pList, param, pOutput);
		return curveList;
	};
